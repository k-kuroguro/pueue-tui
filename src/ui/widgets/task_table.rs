...
pub struct TaskTable {
   tasks: Vec<pueue_lib::Task>,
}

impl TaskTable {
   pub fn new(tasks: &[pueue_lib::Task]) -> Self {
      Self {
         tasks: tasks.to_vec(),
      }
   }

   fn task_to_row<'a>(task: &pueue_lib::Task, header: &[HeaderCell]) -> Row<'a> {
   ...
   fn task_to_cell<'a>(task: &pueue_lib::Task, header_cell: &HeaderCell) -> Cell<'a> {
      match header_cell {
         ...
         HeaderCell::Status => {
            ...
            let style = match &task.status {
               pueue_lib::TaskStatus::Locked { .. } => Style::new(),
               pueue_lib::TaskStatus::Stashed { .. } => Style::new().yellow(),
               pueue_lib::TaskStatus::Queued { .. } => Style::new().yellow(),
               pueue_lib::TaskStatus::Running { .. } => Style::new().green(),
               pueue_lib::TaskStatus::Paused { .. } => Style::new(),
               pueue_lib::TaskStatus::Done { result, .. } => match result {
                  pueue_lib::TaskResult::Success => Style::new().green(),
                  _ => Style::new().red(),
               },
            };
         ...
         HeaderCell::EnqueueAt => {
            let content = if let pueue_lib::TaskStatus::Stashed {
   ...
   fn task_status_to_string(status: &pueue_lib::TaskStatus) -> String {
      match &status {
         pueue_lib::TaskStatus::Done { result, .. } => match result {
            pueue_lib::TaskResult::Success => "Success".to_string(),
            pueue_lib::TaskResult::Failed(code) => format!("Failed ({})", code),
            pueue_lib::TaskResult::FailedToSpawn(_) => "Failed to spawn".to_string(),
            pueue_lib::TaskResult::Killed => "Killed".to_string(),
            pueue_lib::TaskResult::Errored => "Errored".to_string(),
            pueue_lib::TaskResult::DependencyFailed => "Dependency failed".to_string(),
         },
         _ => status.to_string(),
      }
   }
   ...
   fn tasks_to_header(tasks: &[pueue_lib::Task]) -> Vec<HeaderCell> {
   ...
   fn calc_widths(header: &[HeaderCell], tasks: &[pueue_lib::Task]) -> Vec<Constraint> {
      let (
         max_id_width,
         max_status_width,
         max_priority_width,
